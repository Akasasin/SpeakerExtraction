...
class SpeakerClassifier(nn.Module):
    def __init__(self, N=256, P=3, label_length=100):
        super().__init__()
        self.label_length = label_length
        self.batch_norm = nn.BatchNorm1d(3 * N)
        self.conv1 = nn.Conv1d(3 * N, N, 1, bias=False)
        self.resnet_blocks = nn.ModuleList([ResNetBlock(N) for _ in range(P)])
        self.conv2 = nn.Conv1d(N, N, 1, bias=False)
...
class SpExDPConformer(nn.Module):
    def __init__(self, N=256, L1=20, L2=80, L3=160, heads=4, P=3, R=4, segment_size=80, dropout=0.0, label_length=100):
        super().__init__()
        self.encoder = SpeechEncoder(N, L1, L2, L3)
        self.decoder = SpeechDecoder(N, L1, L2, L3)
        self.classifier = SpeakerClassifier(N, P, label_length)
